"""
world_model/train_ssm_nocurr.py — 커리큘럼 없는 SSM 학습 (v16 아키텍처 ablation)

ss=0.0 고정 (pure inference from step 1), rollout_T ~ Uniform(10,60) 매 에폭 랜덤,
w_type=1.0 고정, 충돌 oversampling 없음.

v16 동일 아키텍처 + 동일 손실함수, v15 best.pt에서 fine-tune.
커리큘럼 유무에 따른 성능 차이 비교용 ablation.

Usage:
    python world_model/train_ssm_nocurr.py \
      --ckpt world_model/results/ssm_v15_ar/best.pt \
      --out-dir world_model/results/ssm_v16_nocurr
"""
...
def _eval_batched(model: SSMWorldModel, episodes: list, device: str,
    ...
    """evaluate_rollout_error의 배치 버전 — 에피소드 전체를 한 번에 forward."""
    model.eval()
    checkpoint_steps = {
        "0.5s": int(0.5 / DT),
        "1.0s": int(1.0 / DT),
    ...
    with torch.no_grad():
        ...
        ).float().to(device)                          # (N, 14)

        s_hat, type_logit = model(s0, n_steps=rollout_steps)
        pr = s_hat.cpu().numpy()                      # (N, T+1, 14)
        pc = type_logit.argmax(-1).cpu().numpy() != 0 # (N, T)
...
def train(args):
    ...
    logger.log(f"Mode: no-curriculum  ss=0.0  T~Uniform({T_MIN},{T_MAX})  w_type=1.0"
    ...
    if args.ckpt:
        ...
        saved = raw.get("state", raw)  # v15/v16 ckpt 공통 로드
    ...
    log_sigma = nn.Parameter(torch.zeros(2, device=device))  # Kendall [state, type]
    ...
    for epoch in range(1, args.epochs + 1):
        cur_T = int(rng_T.integers(T_MIN, T_MAX + 1))  # [10, 60]
        ...
        for seq_s, seq_f, seq_t in train_loader:
            ...
            # ss=0.0 → pure inference (gt_states 불필요)
...
def main():
    ...
    p.add_argument("--ckpt",       default="world_model/results/ssm_v15_ar/best.pt",
                   help="fine-tune 시작점 (v15 또는 v16 best.pt)")
    p.add_argument("--lr",            type=float, default=1e-4)
    p.add_argument("--pocket-weight", type=float, default=4.1,
                   help="pocket class weight (class_weights[4])")
    p.add_argument("--focal-gamma",   type=float, default=0.0,
                   help="focal loss gamma (0=standard CE)")
    args = p.parse_args()

    if args.out_dir is None:
        args.out_dir = "world_model/results/ssm_v16_nocurr"
